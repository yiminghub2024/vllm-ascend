...
class AscendMLAImpl(MLAAttentionImpl):
    ...
    def _exec_kv_mla_nope(self, kv_no_split, kv_cache, slots, is_prefill: bool):
        ...
        # so the scatter would silently write nowhere.
        cache = kv_cache[0]
        if not cache.is_contiguous():
            raise RuntimeError(
                f"mla_nope_kv_write: kv cache not contiguous, shape={tuple(cache.shape)} stride={cache.stride()}"
            )
        flat = cache.view(-1, cache.shape[-1])
        token = k_nope.reshape(-1, cache.shape[-1]).to(flat.dtype)
        idx = slots.to(torch.int64)
        flat.index_copy_(0, idx, token)
        if is_prefill:
            return k_pe, k_nope
