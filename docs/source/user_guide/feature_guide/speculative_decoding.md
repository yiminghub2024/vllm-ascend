...
### DFlash2 for GLM-5.3-Flash

`DFlash2DraftModel` checkpoints use the same `"method": "dflash"` entry; the DFlash2 selector path is picked up from the draft model's architecture. [incoai/GLM-5.3-Flash-DFlash2](https://modelscope.cn/models/incoai/GLM-5.3-Flash-DFlash2) is the DFlash2 head published for GLM-5.3-Flash and replaces the built-in MTP head:

```shell
vllm serve ZhipuAI/GLM-5.3-Flash \
  --tensor-parallel-size 4 \
  --block-size 512 \
  --max-model-len 65536 \
  --no-enable-prefix-caching \
...
`num_speculative_tokens` has to be set explicitly. The checkpoint keeps its trained block length in `dflash_config.block_size` instead of a top-level `n_predict`, so vLLM cannot derive it. One token of each block is the verified one, hence `dflash_config.block_size - 1`: 7 for this checkpoint. The drafter reads the target's hidden states at `dflash_config.target_layer_ids`, so the target has to expose auxiliary hidden states; GLM-5.3-Flash does. Unlike the MTP head, which reuses GLM's own decoder layer and is therefore forced into eager mode, this drafter carries a Qwen3-shaped architecture and runs in graph mode.

> [!NOTE]
> The DFlash2 checkpoint is released under CC BY-NC-ND 4.0, for research and evaluation only. Contact its publisher for commercial licensing. The target model's own license is unaffected.
