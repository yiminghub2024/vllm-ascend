...
vllm serve ZhipuAI/GLM-5.3-Flash \
  ...
  --no-enable-prefix-caching \
  --compilation-config '{"cudagraph_mode": "FULL_DECODE_ONLY"}' \
  --speculative-config '{"method": "dflash", "model": "incoai/GLM-5.3-Flash-DFlash2"}'
```

`num_speculative_tokens` defaults to the drafter's `dflash_config.block_size` (8 for this checkpoint) and may be lowered, not raised. The drafter reads the target's hidden states at `dflash_config.target_layer_ids`, so the target has to expose auxiliary hidden states; GLM-5.3-Flash does. Unlike the MTP head, which reuses GLM's own decoder layer and is therefore forced into eager mode, this drafter carries a Qwen3-shaped architecture and runs in graph mode.
